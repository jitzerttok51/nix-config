...
let
  ...
  addIdsToApplets = panels: let
    ...
    in
      cont // (builtins.foldl' fun cont applets);
  ...
  attachRegionToApplets = panels: let
    ...
    in (panel // result);
  in
    builtins.map fun panels;

  toAppletStrings = panels: let
    fun = cont: panel: let
      func = applet: "${panel.name}:${applet.position}:${builtins.toString applet.index}:${applet.type}:${builtins.toString applet.id}";
      map = region: (builtins.map func panel.applets.${region});
      result = (map "left") ++ (map "center") ++ (map "right");
    in
      cont ++ result;
  in
    builtins.foldl' fun [] panels;
